...
from team_assets import team_badge, team_badge_image, team_badge_list, team_theme
...
def nav(prefix: str = "../", current: str = "") -> str:
    links = [
        ...
        ("copa-do-brasil", f"{prefix}copa-do-brasil/", "Copa do Brasil"),
...
def head(
    ...
    theme_color: str = "#101714",
) -> str:
    return f'''<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{esc(title)}</title><meta name="description" content="{esc(description, True)}"><meta name="theme-color" content="{esc(theme_color, True)}"><link rel="canonical" href="{canonical}"><meta property="og:type" content="website"><meta property="og:locale" content="pt_BR"><meta property="og:title" content="{esc(title, True)}"><meta property="og:description" content="{esc(description, True)}"><meta property="og:url" content="{canonical}"><meta property="og:image" content="{social_image}"><meta property="og:image:width" content="1200"><meta property="og:image:height" content="630"><meta property="og:image:alt" content="{esc(social_image_alt, True)}"><meta name="twitter:card" content="summary_large_image"><meta name="twitter:title" content="{esc(title, True)}"><meta name="twitter:description" content="{esc(description, True)}"><meta name="twitter:image" content="{social_image}"><meta name="twitter:image:alt" content="{esc(social_image_alt, True)}"><link rel="stylesheet" href="{depth}style.css"></head>'''


def page(title: str, description: str, canonical: str, content: str, *, depth: str = "../", current: str = "", body_class: str = "br-page", body_style: str = "", script: bool = False, social_image: str = f"{SITE_URL}/assets/og/rodada-a-rodada.png", social_image_alt: str = "Rodada a Rodada — Brasileirão 2026 em números", theme_color: str = "#101714") -> str:
    script_tag = f'<script src="{depth}brasileirao.js" defer></script>' if script else ""
    style_attr = f' style="{esc(body_style, True)}"' if body_style else ""
    return f'{head(title, description, canonical, depth, social_image, social_image_alt, theme_color)}<body class="{body_class}"{style_attr}><a class="skip-link" href="#conteudo">Pular para o conteúdo</a>{nav(depth, current)}<main id="conteudo">{content}</main>{footer(depth)}{script_tag}<script src="{depth}site.js" defer></script></body></html>\n'
...
def home_content(insights: dict[str, Any]) -> str:
    ...
    scorer_text = (
        f'{esc(top_scorer["name"])}<small>{top_scorer["goals"]} gols · {team_badge(top_scorer["team"], "scorer")}</small>'
        if top_scorer else "Em atualização<small>Autores dos gols sendo consolidados</small>"
    )
    ...
    cards = "".join(
        f'<a class="br-team-chip" href="brasileirao/times/{slugify(team)}.html">{team_badge(team, "chip")}</a>'
        for team in insights["teams"]
    )
    rankings = rankings_data(insights)
    return f'''<section class="br-hero"><div class="br-shell br-hero-grid"><div><p class="br-kicker">Brasileirão Série A · {SEASON}</p><h1>A tabela tem memória.</h1><p>Acompanhe quem subiu, quem caiu e como a disputa mudou a cada rodada.</p><div class="br-actions"><a class="br-button br-button-hot" href="brasileirao/classificacao-rodada-a-rodada.html">Explorar classificação</a><a class="br-button" href="brasileirao/rodadas/rodada-{insights['current_round']}.html">Ver a rodada {insights['current_round']}</a></div></div><div class="br-cover" aria-label="Resumo da temporada"><span>EDIÇÃO {insights['current_round']:02d}</span><strong class="{leader_class}">{team_badge(leader['team'], 'cover')}</strong><p>líder com {leader['points']} pontos</p><b>20 TIMES<br>38 RODADAS<br>1 HISTÓRIA</b></div></div></section>
<section class="br-ticker"><div class="br-shell"><span>Rodada {insights['current_round']}</span><strong>{latest['goals']} gols</strong><strong>{team_badge(latest['leader'], 'ticker')} na liderança</strong><strong>{latest['matches']} jogos concluídos</strong></div></section>
<section class="br-section"><div class="br-shell"><div class="br-section-head"><div><p class="br-kicker">Painel da temporada</p><h2>O campeonato agora</h2></div><a href="brasileirao/classificacao-rodada-a-rodada.html">Ver evolução completa →</a></div><div class="br-dashboard"><article class="br-ranking-card"><span>Líder</span><strong class="br-leader-name">{team_badge(leader['team'], 'leader-card')}</strong><b>{leader['points']} pts</b><small>{leader['wins']} vitórias · saldo {leader['gd']:+d}</small></article><article class="br-ranking-card br-ranking-card-hot"><span>Artilharia</span><strong>{scorer_text}</strong><a href="brasileirao/artilharia-rodada-a-rodada.html">Abrir corrida →</a></article><div class="br-table-card">{table_html(current, 6, 'brasileirao/times/')}<a href="brasileirao/classificacao-rodada-a-rodada.html">Tabela completa e rodada a rodada →</a></div></div></div></section>
...
<section class="br-section br-section-dark"><div class="br-shell"><div class="br-section-head"><div><p class="br-kicker">O que mudou</p><h2>Raio-x da rodada {latest['round']}</h2></div><a href="brasileirao/rodadas/rodada-{latest['round']}.html">Ler resumo completo →</a></div><div class="br-change-grid"><article><span>01</span><h3>Liderança</h3><p>{team_badge(latest['leader'], 'sentence')}{' assumiu a ponta.' if latest['leader_changed'] else ' manteve a ponta.'}</p></article><article><span>02</span><h3>Maior subida</h3><p>{team_badge_list(latest['biggest_rise']['teams'], 'sentence', 'Sem mudança')} · {latest['biggest_rise']['places']} posições</p></article><article><span>03</span><h3>Z4</h3><p>Entraram: {team_badge_list(latest['z4_in'], 'sentence')}<br>Saíram: {team_badge_list(latest['z4_out'], 'sentence')}</p></article></div></div></section>
<section class="br-compare-promo"><div class="br-shell br-compare-promo-grid"><div><p class="br-kicker">Novo comparador</p><h2>{team_badge(leader['team'], 'headline')} ou {team_badge(current[1]['team'], 'headline')}: quem fez a melhor campanha?</h2><p>Coloque dois times frente a frente e acompanhe a disputa rodada a rodada.</p></div><a class="br-button" href="brasileirao/comparador-times.html">Comparar times</a></div></section>
...
def scorers_content(insights: dict[str, Any]) -> str:
    ...
    options = "".join(f'<option value="{n}"{(" selected" if n == insights["current_round"] else "")}>Rodada {n}</option>' for n in range(1, insights["current_round"] + 1))
    rows = "".join(
        f'<tr><td>{i}</td><th scope="row">{esc(row["name"])}</th><td><a href="times/{slugify(row["team"])}.html">{team_badge(row["team"], "table")}</a></td><td><strong>{row["goals"]}</strong></td></tr>'
...
def main() -> None:
    ...
    leader_team = insights["snapshots"][-1]["table"][0]["team"]
    leader_color, leader_text = team_theme(leader_team)
    leader_style = f"--br-leader-color:{leader_color};--br-leader-text:{leader_text}"
    home = page("Rodada a Rodada | Brasileirão 2026 em números", "Veja a evolução da classificação, artilharia, times e rodadas do Brasileirão 2026.", f"{SITE_URL}/", home_content(insights), depth="", body_class="br-page br-home br-leader-theme", body_style=leader_style, script=True, theme_color=leader_color)
    write(ROOT / "index.html", home)
    write(OUTPUT / "index.html", page("Brasileirão 2026 rodada a rodada", "Explore classificação, artilharia, times e todas as rodadas do Brasileirão 2026.", f"{SITE_URL}/brasileirao/", hub_content(insights), body_class="br-page br-leader-theme", body_style=leader_style, theme_color=leader_color))
